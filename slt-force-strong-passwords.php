<?php
/**
 * Plugin Name:  Cleancoded WP Strong Passwords
 * Plugin URI:   https://github.com/cleancoded/wp-strong-passwords/
...
if ( ! function_exists( 'add_action' ) ) {
	esc_html_e( "Hi there! I'm just a plugin, not much I can do when called directly.", 'cleancoded-force-strong-passwords' );
	exit;
}
...
function slt_fsp_init() {
	...
	load_plugin_textdomain( 'cleancoded-force-strong-passwords', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );

	// Hooks.
	add_action( 'user_profile_update_errors', 'cleancoded_fsp_validate_profile_update', 0, 3 );
	add_action( 'validate_password_reset', 'cleancoded_fsp_validate_strong_password', 10, 2 );
	add_action( 'resetpass_form', 'cleancoded_fsp_validate_resetpass_form', 10 );
 ...
 */
function cleancoded_fsp_validate_profile_update( $errors, $update, $user_data ) {
	return cleancoded_fsp_validate_strong_password( $errors, $user_data );
}

/**
 * Check password reset form and throw an error if the password isn't strong.
 */
function cleancoded_fsp_validate_resetpass_form( $user_data ) {
	return cleancoded_fsp_validate_strong_password( false, $user_data );
}
 ...
 */
function cleancoded_fsp_validate_strong_password( $errors, $user_data ) {
	$password_ok = true;
	$enforce     = true;
	...
	if ( ! $password_ok && is_wp_error( $errors ) ) { // Is this a WP error object?
		$errors->add( 'pass', apply_filters( 'slt_fsp_error_message', __( '<strong>ERROR</strong>: Please make the password a strong one.', 'cleancoded-force-strong-passwords' ) ) );
	}
